...
describe("billing recheck webhook receipt", () => {
  it("includes webhook receipt and dedupe", async () => {
    vi.useFakeTimers();
    ...
    expect(body.model.webhookReceipt).toBeDefined();
    expect(body.model.webhookReceipt.dedupe.distinctCount24h).toBeGreaterThan(0);
    expect(body.model.webhookDedupe).toBeDefined();
    vi.useRealTimers();
  });
});
