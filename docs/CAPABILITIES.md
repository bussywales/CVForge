...
Ops Case View: /app/ops/case provides a requestId-first cockpit with search + window selector, alerts/incidents/audits/webhooks/billing/resolution/watch panels, copyable case snippets, and canonical request context mapping (requestId → userId/email) with auto-resolve + confidence to unlock billing/dossier panels.
