...
## Application Packs
Application Packs let users create a job-specific bundle from a JD plus optional CV/notes. Each pack stores version history, produces tailored CV and cover letter content, STAR suggestions, a fit map with gaps, and a “what changed” summary, with DOCX export in Standard and ATS-clean formats. The pack detail page includes generation progress, version switching, and per-tab copy actions, and export is gated until a version exists.
...
Ops Case Queue: /app/ops/cases provides a requestId-first inbox with per-user saved views (save/manage/default), SLA due/breached badges + breach filtering/sort, reason badges with sources roll-ups, quick actions (claim/status/priority), return-to-queue links, auto-refresh freshness indicators, and a My queue workload summary for managing assigned vs unassigned cases.
