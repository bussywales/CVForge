...
export async function POST(request: Request) {
  ...
  const budget = getRateLimitBudget("ops_case_close");
  const limiter = checkRateLimit({
    route: "ops_case_close",
    identifier: user.id,
    limit: budget.limit,
  ...
  if (!limiter.allowed) {
    const res = jsonError({
      ...
      requestId,
      status: 429,
      meta: { limitKey: "ops_case_close", budget: budget.budget, retryAfterSeconds: limiter.retryAfterSeconds },
    });
  ...
  try {
    const row = await updateCaseStatus({ requestId: caseRequestId, status: "closed" });
    const admin = createServiceRoleClient();
    const meta = sanitizeCaseWorkflowMeta({ requestId: caseRequestId, status: row.status });
    ...
    captureServerError(error, { requestId, route: "/api/ops/case/close", code: "OPS_CASE_CLOSE_FAIL" });
    return jsonError({ code: "OPS_CASE_CLOSE_FAIL", message: "Unable to close case", requestId });
  }
}
