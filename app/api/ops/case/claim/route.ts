...
import { claimCaseWorkflow, sanitizeCaseWorkflowMeta } from "@/lib/ops/ops-case-workflow";
...
export async function POST(request: Request) {
  ...
  const budget = getRateLimitBudget("ops_case_claim");
  const limiter = checkRateLimit({
    route: "ops_case_claim",
    identifier: user.id,
    limit: budget.limit,
  ...
  if (!limiter.allowed) {
    const res = jsonError({
      ...
      requestId,
      status: 429,
      meta: { limitKey: "ops_case_claim", budget: budget.budget, retryAfterSeconds: limiter.retryAfterSeconds },
    });
  ...
  try {
    const { row, conflict } = await claimCaseWorkflow({ requestId: caseRequestId, actorUserId: user.id });
    if (conflict) {
      const res = jsonError({
        code: "CASE_CONFLICT",
        message: "Case already claimed",
        requestId,
        status: 409,
        meta: {
      ...
      try {
        await logMonetisationEvent(admin as any, user.id, "ops_case_conflict", {
          meta: { status: row.status, hasOwner: Boolean(row.assigned_to_user_id) },
        });
      } catch {
        // ignore
      }
    ...
    try {
      await logMonetisationEvent(admin as any, user.id, "ops_case_claim", {
    ...
    captureServerError(error, { requestId, route: "/api/ops/case/claim", code: "OPS_CASE_CLAIM_FAIL" });
    return jsonError({ code: "OPS_CASE_CLAIM_FAIL", message: "Unable to claim case", requestId });
  }
}
